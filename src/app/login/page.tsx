  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-8">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-20 -left-20 w-72 h-72 bg-emerald-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20" />
      ...
      <div className="w-full max-w-md relative">
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-emerald-100 p-6 sm:p-8 animate-slide-up">
          <div className="text-center mb-8">
            <div className="gradient-primary inline-flex rounded-2xl p-3 text-white shadow-lg shadow-emerald-500/20 mb-4">
              <LogIn size={24} />
            </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full gradient-primary text-white py-2.5 sm:py-3 rounded-full font-bold text-sm sm:text-base hover:shadow-lg hover:shadow-emerald-500/25 transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-[0.98]"
            >
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            New student?{" "}
            <Link href="/register" className="font-extrabold bg-gradient-to-r from-emerald-600 to-emerald-500 bg-clip-text text-transparent hover:underline">
              Create an account
            </Link>
          </p>
        </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-[80vh] flex items-center justify-center"><Loader2 size={32} className="animate-spin text-emerald-600" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
